...
struct AddressHelper {
    ...
    #[serde(alias = "city")]
    #[serde(rename = "value-tag=city")]
    city: Option<String>,

    #[serde(alias = "postalCode")]
    #[serde(rename = "value-tag=postalCode")]
    postal_code: Option<String>,

    #[serde(alias = "country")]
    #[serde(rename = "value-tag=country")]
...
impl AddressDef {
    pub fn serialize<S: Serializer>(address: &Address, serializer: S) -> Result<S::Ok, S::Error> {
        let value: AddressHelper = address.clone().into();
    ...
    fn into(self) -> AddressHelper {
        ...
        if let Some(number) = self.number {
            extension.push(ExtensionDef {
                ...
                value: Some(ValueDef::String(number.into())),
                ..Default::default()
            })
        }
        ...
        if let Some(street) = self.street {
            extension.push(ExtensionDef {
                ...
                value: Some(ValueDef::String(street.into())),
                ..Default::default()
            })
        }
        ...
        if let Some(addition) = self.addition {
            extension.push(ExtensionDef {
                ...
                value: Some(ValueDef::String(addition.into())),
                ..Default::default()
            })
        }
        ...
        if let Some(post_box) = self.post_box {
            extension.push(ExtensionDef {
                ...
                value: Some(ValueDef::String(post_box.into())),
                ..Default::default()
            })
        }
...
impl TryInto<Address> for AddressHelper {
    ...
    fn try_into(self) -> Result<Address, Self::Error> {
        ...
        for ex in line.extension {
            match ex.url.as_str() {
                EXTENSION_URL_ADDRESS_STREET => match ex.value {
                    Some(ValueDef::String(value)) => street = Some(value.into()),
                    _ => {
                        return Err(
                ...
                EXTENSION_URL_ADDRESS_NUMBER => match ex.value {
                    Some(ValueDef::String(value)) => number = Some(value.into()),
                    _ => {
                        return Err(
                ...
                EXTENSION_URL_ADDRESS_ADDITION => match ex.value {
                    Some(ValueDef::String(value)) => addition = Some(value.into()),
                    _ => {
                        return Err(
                ...
                EXTENSION_URL_ADDRESS_POST_BOX => match ex.value {
                    Some(ValueDef::String(value)) => post_box = Some(value.into()),
                    _ => {
                        return Err(
