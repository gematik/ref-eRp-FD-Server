...
struct NameHelper {
    ...
    #[serde(alias = "given")]
    #[serde(rename = "value-tag=given")]
    given: String,

    #[serde(default)]
    prefix: Option<PrefixDef>,
}
    ...
    fn into(self) -> NameHelper {
        ...
        if let Some(family) = self.family {
            extension.push(ExtensionDef {
                ...
                value: Some(ValueDef::String(family.into())),
                ..Default::default()
            })
        }
        ...
        if let Some(family_ext) = self.family_ext {
            extension.push(ExtensionDef {
                ...
                value: Some(ValueDef::String(family_ext.into())),
                ..Default::default()
            })
        }
        ...
        if let Some(family_prefix) = self.family_prefix {
            extension.push(ExtensionDef {
                ...
                value: Some(ValueDef::String(family_prefix.into())),
                ..Default::default()
            })
        }

        let prefix_qualifier = self.prefix_qualifier;
        ...
        NameHelper {
            ...
            given: self.given,
            prefix: self.prefix.map(|prefix| PrefixDef {
                value: prefix,
                extension: if prefix_qualifier {
                    vec![ExtensionDef {
...
impl TryInto<Name> for NameHelper {
    ...
    fn try_into(self) -> Result<Name, Self::Error> {
        ...
        let name = self.family.value;
        let mut family = None;
        let mut family_ext = None;
        let mut family_prefix = None;

        for ex in self.family.extension {
            match ex.url.as_str() {
                EXTENSION_URL_HUMAN_NAME_OWN_NAME => match ex.value {
                    Some(ValueDef::String(value)) => family = Some(value.into()),
                    _ => {
                        return Err(
                ...
                EXTENSION_URL_HUMAN_NAME_EXTENSION => match ex.value {
                    Some(ValueDef::String(value)) => family_ext = Some(value.into()),
                    _ => {
                        return Err(
                ...
                EXTENSION_URL_HUMAN_NAME_PREFIX => match ex.value {
                    Some(ValueDef::String(value)) => family_prefix = Some(value.into()),
                    _ => {
                        return Err(
            ...
            for ex in p.extension {
                match ex.url.as_str() {
                    EXTENSION_URL_ISO21090_EN => {
                        match &ex.value {
                            Some(ValueDef::Code(code)) if code.0 == "AC" => (),
                            Some(_) => {
