...
use super::{
    super::super::constants::{
        ...
        IDENTITY_SYSTEM_IKNR, RESOURCE_PROFILE_COVERAGE, XMLNS_COVERAGE,
    },
    misc::{
...
impl TryInto<Coverage> for CoverageHelper {
    ...
    fn try_into(self) -> Result<Coverage, Self::Error> {
        if self.meta.profile != vec![RESOURCE_PROFILE_COVERAGE.into()] {
...
pub mod tests {
    ...
    fn convert_to() {
        ...
        let actual = trim_xml_str(&to_xml(&CoverageRoot::new(&coverage)).unwrap());
        let expected = trim_xml_str(&read_to_string("./examples/coverage.xml").unwrap());
    ...
    #[test]
    fn convert_from() {
        let actual = from_xml::<CoverageRoot>(&read_to_string("./examples/coverage.xml").unwrap())
            .unwrap()
            .into_inner();
