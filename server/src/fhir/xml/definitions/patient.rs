...
use super::{
    super::super::constants::{
        ...
        RESOURCE_PROFILE_PATIENT, XMLNS_PATIENT,
    },
    misc::{
...
pub mod tests {
    ...
    fn convert_to() {
        ...
        let actual = trim_xml_str(&to_xml(&PatientRoot::new(&patient)).unwrap());
        let expected = trim_xml_str(&read_to_string("./examples/patient.xml").unwrap());
    ...
    #[test]
    fn convert_from() {
        let actual = from_xml::<PatientRoot>(&read_to_string("./examples/patient.xml").unwrap())
            .unwrap()
            .into_inner();
