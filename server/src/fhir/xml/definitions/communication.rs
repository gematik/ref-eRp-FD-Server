...
use super::{
    super::super::constants::{
        ...
        RESOURCE_PROFILE_COMMUNICATION_REPRESENTATIVE, XMLNS_COMMUNICATION,
    },
    misc::{
        CodingDef, DeserializeRoot, ExtensionDef, IdentifierDef, MetaDef, ReferenceDef, Root,
...
struct PayloadDef {
    ...
    #[serde(alias = "contentString")]
    #[serde(rename = "value-tag=contentString")]
    content: String,
}
...
impl XmlnsType for Communication {
    fn xmlns() -> &'static str {
        XMLNS_COMMUNICATION
    }
}
    ...
    fn into(self) -> PayloadExtensionsDef {
        let mut ret = vec![
            ...
            ExtensionDef {
                ...
                value: Some(ValueDef::Boolean(self.substitution_allowed.into())),
                ..Default::default()
            },
            ExtensionDef {
    ...
    fn into(self) -> ExtensionDef {
        ExtensionDef {
            ...
            extension: vec![
                ExtensionDef {
                    url: "onPremise".into(),
                    value: Some(ValueDef::Boolean(self.on_premise.into())),
                    ..Default::default()
                },
                ExtensionDef {
                    url: "delivery".into(),
                    value: Some(ValueDef::Boolean(self.delivery.into())),
                    ..Default::default()
                },
                ExtensionDef {
                    url: "shipment".into(),
                    value: Some(ValueDef::Boolean(self.shipment.into())),
                    ..Default::default()
                },
            ],
            ..Default::default()
...
pub mod tests {
    ...
    fn convert_to_info_req() {
        ...
        let actual = trim_xml_str(&to_xml(&CommunicationRoot::new(&communication)).unwrap());
        let expected =
            trim_xml_str(&read_to_string("./examples/communication_info_req.xml").unwrap());
    ...
    fn convert_to_reply() {
        ...
        let actual = trim_xml_str(&to_xml(&CommunicationRoot::new(&communication)).unwrap());
        let expected = trim_xml_str(&read_to_string("./examples/communication_reply.xml").unwrap());
    ...
    fn convert_to_dispense_req() {
        ...
        let actual = trim_xml_str(&to_xml(&CommunicationRoot::new(&communication)).unwrap());
        let expected =
            trim_xml_str(&read_to_string("./examples/communication_dispense_req.xml").unwrap());
    ...
    fn convert_from_info_req() {
        let xml = read_to_string("./examples/communication_info_req.xml").unwrap();
        let actual = from_xml::<CommunicationRoot>(&xml).unwrap().into_inner();
    ...
    fn convert_from_reply() {
        let xml = read_to_string("./examples/communication_reply.xml").unwrap();
        let actual = from_xml::<CommunicationRoot>(&xml).unwrap().into_inner();
    ...
    fn convert_from_dispense_req() {
        let xml = read_to_string("./examples/communication_dispense_req.xml").unwrap();
        let actual = from_xml::<CommunicationRoot>(&xml).unwrap().into_inner();
