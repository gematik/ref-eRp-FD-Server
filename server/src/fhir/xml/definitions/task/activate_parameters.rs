...
use super::super::{
    super::super::constants::{
        BINARY_CONTENT_TYPE_PKCS7, PARAMETER_TYPE_TASK_ACTIVATE, XMLNS_PARAMETERS,
    },
    misc::{BinaryDef, DeserializeRoot, ParametersDef, Root, SerializeRoot, XmlnsType},
};
...
impl XmlnsType for TaskActivateParameters {
    fn xmlns() -> &'static str {
        XMLNS_PARAMETERS
    }
}
...
pub mod tests {
    ...
    #[test]
    fn convert_from() {
        let data = read_to_string("./examples/task_activate_parameters.xml").unwrap();
        let actual = from_xml::<TaskActivateParametersRoot>(&data)
            .unwrap()
            .into_inner();
    ...
    #[test]
    fn convert_from_erfd_130() {
        let data = read_to_string("./examples/task_activate_parameters_erfd-130.xml").unwrap();
        let ret = from_xml::<TaskActivateParametersRoot>(&data);
