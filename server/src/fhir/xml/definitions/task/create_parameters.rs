...
use super::super::{
    super::super::constants::{
        CODING_SYSTEM_FLOW_TYPE, PARAMETER_TYPE_TASK_CREATE, XMLNS_PARAMETERS,
    },
    misc::{
        CodingDef, DeserializeRoot, ParameterDef, ParametersDef, Root, SerializeRoot, ValueDef,
        XmlnsType,
    },
};
...
impl XmlnsType for TaskCreateParameters {
    fn xmlns() -> &'static str {
        XMLNS_PARAMETERS
    }
}
...
pub mod tests {
    ...
    fn convert_to() {
        ...
        let actual = trim_xml_str(&to_xml(&TaskCreateParametersRoot::new(&task)).unwrap());
        let expected =
            trim_xml_str(&read_to_string("./examples/task_create_parameters.xml").unwrap());
    ...
    #[test]
    fn convert_from() {
        let actual = from_xml::<TaskCreateParametersRoot>(
            &read_to_string("./examples/task_create_parameters.xml").unwrap(),
        )
        .unwrap()
        .into_inner();
