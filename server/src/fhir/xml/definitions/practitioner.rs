...
impl XmlnsType for Practitioner {
    fn xmlns() -> &'static str {
        XMLNS_PRACTITIONER
    }
}
...
pub mod tests {
    ...
    fn convert_to() {
        ...
        let actual = trim_xml_str(&to_xml(&PractitionerRoot::new(&practitioner)).unwrap());
        let expected = trim_xml_str(&read_to_string("./examples/practitioner.xml").unwrap());
    ...
    fn convert_from() {
        let actual =
            from_xml::<PractitionerRoot>(&read_to_string("./examples/practitioner.xml").unwrap())
                .unwrap()
                .into_inner();
