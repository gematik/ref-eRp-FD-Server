...
use super::{
    super::super::constants::{
        ...
        RESOURCE_PROFILE_MEDICATION_INGREDIENT, RESOURCE_PROFILE_MEDICATION_PZN,
        RESOURCE_TYPE_MEDICATION,
    },
    misc::{
        CodableConceptDef, CodingDef, DeserializeRoot, ExtensionDef, MetaDef, QuantityDef,
        RatioDef, ResourceType, SerializeRoot, ValueDef,
    },
    primitives::IdDef,
};
...
impl ResourceType for Medication {
...
fn serialize_extension(value: &Medication) -> Result<Vec<ExtensionDef>, String> {
    ...
    let mut result = vec![
        ...
        ExtensionDef {
            ...
            value: Some(ValueDef::Boolean(extension.vaccine)),
            ..Default::default()
        },
    ];
    ...
    if let Some(instruction) = &extension.instruction {
        result.push(ExtensionDef {
            ...
            value: Some(ValueDef::String(instruction.clone())),
            ..Default::default()
        });
    }
    ...
    if let Some(packaging) = &extension.packaging {
        result.push(ExtensionDef {
            ...
            value: Some(ValueDef::String(packaging.clone())),
            ..Default::default()
        });
    }
    ...
    if let Some(standard_size) = &extension.standard_size {
        result.push(ExtensionDef {
            ...
            value: Some(ValueDef::Code(standard_size.encode_str())),
            ..Default::default()
        });
    }
            ...
            .map(|ingredient| {
                ...
                if let Some(dosage_form) = &ingredient.dosage_form {
                    extension.push(ExtensionDef {
                        ...
                        value: Some(ValueDef::String(dosage_form.clone())),
                        ..Default::default()
                    })
                }
                ...
                if let Some(amount_free_text) = &ingredient.amount_free_text {
                    strength.extension.push(ExtensionDef {
                        ...
                        value: Some(ValueDef::String(amount_free_text.clone())),
                        ..Default::default()
                    })
                }
        ...
        for ex in ingredient.extension {
            match ex.url.as_str() {
                EXTENSION_URL_MEDICATION_INGREDIENT_FORM => match ex.value {
                    Some(ValueDef::String(value)) => dosage_form = Some(value),
                    _ => {
                        return Err(
        ...
        for ex in ingredient.strength.extension {
            match ex.url.as_str() {
                EXTENSION_URL_MEDICATION_INGREDIENT_AMOUNT => match ex.value {
                    Some(ValueDef::String(value)) => amount_free_text = Some(value),
                    _ => {
                        return Err(
        ...
        match ex.url.as_str() {
            ...
            EXTENSION_URL_MEDICATION_VACCINE => {
                vaccine = match ex.value {
                    Some(ValueDef::Boolean(value)) => Some(value),
                    _ => {
                        return Err(
            ...
            EXTENSION_URL_MEDICATION_INSTRUCTION => {
                instruction = match ex.value {
                    Some(ValueDef::String(value)) => Some(value),
                    _ => {
                        return Err(
            ...
            EXTENSION_URL_STANDARD_SIZE => {
                standard_size = match ex.value {
                    Some(ValueDef::Code(code)) => {
                        Some(StandardSize::decode(code).map_err(|err| {
...
pub mod tests {
    ...
    fn convert_to_compounding() {
        ...
        let actual = trim_json_str(&to_json(&MedicationRoot::new(&medication)).unwrap());
        let expected =
            trim_json_str(&read_to_string("./examples/medication_compounding.json").unwrap());
    ...
    fn convert_to_free_text() {
        ...
        let actual = trim_json_str(&to_json(&MedicationRoot::new(&medication)).unwrap());
        let expected =
            trim_json_str(&read_to_string("./examples/medication_free_text.json").unwrap());
    ...
    fn convert_to_ingredient() {
        ...
        let actual = trim_json_str(&to_json(&MedicationRoot::new(&medication)).unwrap());
        let expected =
            trim_json_str(&read_to_string("./examples/medication_ingredient.json").unwrap());
    ...
    fn convert_to_pzn() {
        ...
        let actual = trim_json_str(&to_json(&MedicationRoot::new(&medication)).unwrap());
        let expected = trim_json_str(&read_to_string("./examples/medication_pzn.json").unwrap());
    ...
    fn convert_from_compounding() {
        let xml = read_to_string("./examples/medication_compounding.json").unwrap();
        let actual = from_json::<MedicationRoot>(&xml).unwrap().into_inner();
    ...
    fn convert_from_free_text() {
        let xml = read_to_string("./examples/medication_free_text.json").unwrap();
        let actual = from_json::<MedicationRoot>(&xml).unwrap().into_inner();
    ...
    fn convert_from_ingredient() {
        let xml = read_to_string("./examples/medication_ingredient.json").unwrap();
        let actual = from_json::<MedicationRoot>(&xml).unwrap().into_inner();
    ...
    #[test]
    fn convert_from_pzn() {
        let xml = read_to_string("./examples/medication_pzn.json").unwrap();
        let actual = from_json::<MedicationRoot>(&xml).unwrap().into_inner();
