...
use super::{
    super::super::constants::{
        ...
        RESOURCE_PROFILE_ORGANIZATION, RESOURCE_TYPE_ORGANIZATION,
    },
    misc::{
        AddressCow, CodableConceptDef, CodingDef, ContactPointDef, DeserializeRoot, IdentifierDef,
        MetaDef, ResourceType, SerializeRoot,
    },
    primitives::IdDef,
};
...
pub mod tests {
    ...
    fn convert_to() {
        ...
        let actual = trim_json_str(&to_json(&OrganizationRoot::new(&organization)).unwrap());
        let expected = trim_json_str(&read_to_string("./examples/organization.json").unwrap());
    ...
    fn convert_from() {
        let actual =
            from_json::<OrganizationRoot>(&read_to_string("./examples/organization.json").unwrap())
                .unwrap()
                .into_inner();
