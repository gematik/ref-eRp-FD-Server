...
use super::{
    super::super::constants::{
        IDENTIFIER_SYSTEM_PRESCRIPTION_ID, RESOURCE_PROFILE_KBV_BUNDLE, RESOURCE_TYPE_KBV_BUNDLE,
    },
    misc::{DeserializeRoot, IdentifierDef, MetaDef, ResourceType, SerializeRoot},
    primitives::{IdDef, InstantDef},
...
struct KbvBundleHelper<'a> {
    ...
    #[serde(with = "InstantDef")]
    pub timestamp: Instant,

    pub entry: Vec<EntryDef<'a>>,
}
...
impl ResourceType for KbvBundle {
    fn resource_type() -> &'static str {
        RESOURCE_TYPE_KBV_BUNDLE
    }
}
...
pub mod tests {
    ...
    use super::super::{
        ...
        misc::Root, organization::tests::test_organization, patient::tests::test_patient,
        practitioner::tests::test_practitioner, practitioner_role::tests::test_practitioner_role,
    };

    type KbvBundleRoot<'a> = Root<KbvBundleCow<'a>>;

    #[test]
    fn convert_to() {
        let bundle = test_kbv_bundle();

        let actual = trim_json_str(&to_json(&KbvBundleRoot::new(&bundle)).unwrap());
        let expected = trim_json_str(&read_to_string("./examples/kbv_bundle.json").unwrap());
    ...
    fn convert_from() {
        let actual =
            from_json::<KbvBundleRoot>(&read_to_string("./examples/kbv_bundle.json").unwrap())
                .unwrap()
                .into_inner();
