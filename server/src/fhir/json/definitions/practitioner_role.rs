...
use super::{
    super::super::constants::{
        IDENTITY_SYSTEM_TEAM_NUMBER, RESOURCE_PROFILE_PRACTITIONER_ROLE,
        RESOURCE_TYPE_PRACTITIONER_ROLE,
    },
    misc::{DeserializeRoot, IdentifierDef, MetaDef, ReferenceDef, ResourceType, SerializeRoot},
    primitives::IdDef,
};
...
pub mod tests {
    ...
    fn convert_to() {
        ...
        let actual = trim_json_str(&to_json(&PractitionerRoleRoot::new(&bundle)).unwrap());
        let expected = trim_json_str(&read_to_string("./examples/practitioner_role.json").unwrap());
    ...
    #[test]
    fn convert_from() {
        let json = read_to_string("./examples/practitioner_role.json").unwrap();
        let actual = from_json::<PractitionerRoleRoot>(&json)
            .unwrap()
            .into_inner();
