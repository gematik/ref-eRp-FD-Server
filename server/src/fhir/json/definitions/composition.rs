...
use super::{
    super::super::constants::{
        ...
        RESOURCE_PROFILE_COMPOSITION, RESOURCE_TYPE_COMPOSITION,
    },
    misc::{
...
impl ResourceType for Composition {
    fn resource_type() -> &'static str {
        RESOURCE_TYPE_COMPOSITION
    }
}
...
impl TryInto<Composition> for CompositionHelper {
    ...
    fn try_into(self) -> Result<Composition, Self::Error> {
        if self.meta.profile != vec![RESOURCE_PROFILE_COMPOSITION.to_owned()] {
...
pub mod tests {
    ...
    fn convert_to() {
        ...
        let actual = trim_json_str(&to_json(&CompositionRoot::new(&bundle)).unwrap());
        let expected = trim_json_str(&read_to_string("./examples/composition.json").unwrap());
    ...
    #[test]
    fn convert_from() {
        let json = read_to_string("./examples/composition.json").unwrap();
        let actual = from_json::<CompositionRoot>(&json).unwrap().into_inner();
