...
use super::{
    super::super::constants::{
        ...
        IDENTITY_SYSTEM_KVID, RESOURCE_PROFILE_TASK, RESOURCE_TYPE_TASK, TASK_INTENT,
    },
    misc::{
        CodableConceptDef, CodingDef, DeserializeRoot, ExtensionDef, IdentifierDef, MetaDef,
        ReferenceDef, ResourceType, Root, SerializeRoot, ValueDef,
    },
...
impl TryInto<Task> for TaskHelper {
    ...
    fn try_into(self) -> Result<Task, String> {
        ...
        if meta.profile != vec![RESOURCE_PROFILE_TASK.to_owned()] {
...
pub mod tests {
    ...
    fn convert_to() {
        ...
        let actual = trim_json_str(&to_json(&TaskRoot::new(&task)).unwrap());
        let expected = trim_json_str(&read_to_string("./examples/task.json").unwrap());
    ...
    #[test]
    fn convert_from() {
        let actual = from_json::<TaskRoot>(&read_to_string("./examples/task.json").unwrap())
            .unwrap()
            .into_inner();
