...
use super::{
    super::super::constants::{
        ...
        RESOURCE_PROFILE_MEDICATION_REQUEST, RESOURCE_TYPE_MEDICATION_REQUEST,
    },
    misc::{
        CodingDef, DeserializeRoot, ExtensionDef, MetaDef, QuantityDef, ReferenceDef, ResourceType,
        SerializeRoot, ValueDef,
    },
...
impl ResourceType for MedicationRequest {
    ...
    fn into(self) -> ExtensionsDef {
        let mut ret = vec![
            ExtensionDef {
                ...
                value: Some(ValueDef::Boolean(self.emergency_service_fee)),
                ..Default::default()
            },
            ExtensionDef {
                url: EXTENSION_URL_BGV.into(),
                value: Some(ValueDef::Boolean(self.bvg)),
                ..Default::default()
            },
        ];
        ...
        if let Some(accident_information) = &self.accident_information {
            ...
            if let Some(business) = &accident_information.business {
                extension.push(ExtensionDef {
                    ...
                    value: Some(ValueDef::String(business.clone())),
                    ..Default::default()
                });
            }
        ...
        DosageInstructionDef {
            extension: self
                ...
                .map(|flag| ExtensionDef {
                    ...
                    value: Some(ValueDef::Boolean(flag)),
                    ..Default::default()
                })
                .collect(),
    ...
    fn try_into(self) -> Result<Extension, Self::Error> {
        ...
        for ex in self.0 {
            if ex.url == EXTENSION_URL_EMERGENCY_SERVICE_FEE {
                match ex.value {
                    Some(ValueDef::Boolean(value)) => emergency_service_fee = Some(value),
                    _ => {
                        return Err(
                ...
                match ex.value {
                    Some(ValueDef::Boolean(value)) => bvg = Some(value),
                    _ => {
                ...
                for ex in ex.extension {
                    if ex.url == EXTENSION_URL_ACCIDENT_CAUSE {
                        ...
                        match ex.value {
                            Some(ValueDef::String(value)) => business = Some(value),
                            _ => return Err(
...
pub mod tests {
    ...
    fn convert_to() {
        ...
        let actual = trim_json_str(&to_json(&MedicationRequestRoot::new(&bundle)).unwrap());
        let expected =
            trim_json_str(&read_to_string("./examples/medication_request.json").unwrap());
    ...
    #[test]
    fn convert_from() {
        let xml = read_to_string("./examples/medication_request.json").unwrap();
        let actual = from_json::<MedicationRequestRoot>(&xml)
            .unwrap()
            .into_inner();
